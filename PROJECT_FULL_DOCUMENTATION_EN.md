...
The site is now installable on mobile and desktop like a real app, using the site's own favicon as the app icon. **Full details are in a separate file: `PWA_DOCUMENTATION_AR.md`** (line-by-line explanation of `manifest.json`, `sw.js`, and how the Service Worker registration works).
